...
def baostock_login_context():
    ...
    # 重定向标准输出以抑制登录/登出消息
    original_stdout_fd = sys.stdout.fileno()        # 获取标准输出文件描述符
    saved_stdout_fd = os.dup(original_stdout_fd)        # 复制标准输出文件描述符
    devnull_fd = os.open(os.devnull, os.O_WRONLY)       # 打开 /dev/null（Linux/Mac）

    os.dup2(devnull_fd, original_stdout_fd)         # 定位到黑洞
    os.close(devnull_fd)

    logger.debug("Attempting Baostock login...")
    lg = bs.login()     # 这里的打印消息不会显示在控制台
